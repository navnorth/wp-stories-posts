			...
			<?php
			    ...
			    echo '</div>';
					...
					else
					{
                        ...
                        <div class="col-md-8 col-sm-12 col-xs-12 pblctn_lft_sid_img_cntnr map_cntnr">
                            ...
                            <header class="tax-header">
                                <h1 class="tax-title">
                                     <?php printf( __( 'Results: %s', SCP_SLUG ), '<i>'.$termobject->name.'</i> <span>(0 Stories)</span>' );?>
                                </h1>
                                <div class="topics-search-box">
                                    <form method="get">
                                        <input type="hidden" name="action" value="search" />
				...
				<?php get_sort_box($postids); ?>
						...
						if ($post_count>10 & $paged<$max_page) {
							...
							echo '<div class="col-md-12 pblctn_paramtr padding_left"><a href="?page='.($paged+1).'" data-page-number="'.($paged+1).'" data-page="characteristics" data-base-url="'.$base_url.'" data-max-page="'.$max_page.'" data-posts="'.json_encode($postids).'" class="btn-load-more">Load More</a></div>';		
						}
						?>
					</div>
                    <?php
